...
from newsreposter.core import media_parsers
...
async def aiogram_post_item(
    ...
    *,
    ...
    full_text = (
        f"<b><u>{title}</u></b>\n\n{description}\n\n{link}"
        if description
        else f"<b><u>{title}</u></b>\n\n{link}"
    )

    media = await asyncio.to_thread(media_parsers.parse, link)

    if len(full_text) >= full_post_max_len:
        short_text = f"<b><u>{title}</u></b>\n\n{link}"
        to_send = short_text
    ...
    if isinstance(media, dict):
        photos = list(media.get("photo") or [])
        videos = list(media.get("video") or [])
    elif isinstance(media, (list, tuple)) and media:
        photos = list(media)
    else:
        logger.debug("Unknown media format or empty: {}", type(media))
    ...
    try:
        if videos:
            ...
            for i, v in enumerate(videos[:10]):
                if i == 0:
                    input_media.append(InputMediaVideo(media=v, caption=to_send, parse_mode="HTML"))
                else:
                    input_media.append(InputMediaVideo(media=v))
            await bot.send_media_group(chat_id, input_media)
            logger.info("Posted to TG (media_group, videos: {}): {}", len(videos), title)
            return

        if photos:
            if len(photos) == 1:
                await bot.send_photo(chat_id, photos[0], caption=to_send, parse_mode="HTML")
                logger.info("Posted to TG (photo): {}", title)
            else:
                input_media = []
                for i, p in enumerate(photos[:10]):
                    if i == 0:
                        input_media.append(InputMediaPhoto(media=p, caption=to_send, parse_mode="HTML"))
                    else:
                        input_media.append(InputMediaPhoto(media=p))
                await bot.send_media_group(chat_id, input_media)
                logger.info("Posted to TG (media_group, photos: {}): {}", len(photos), title)
            return

        await bot.send_message(chat_id, to_send, parse_mode="HTML", disable_web_page_preview=disable_preview)
        logger.info("Posted to TG (text only): {}", title)

    except TelegramAPIError:
        logger.exception("Telegram API error while posting item: {}", title)
        await bot.send_message(chat_id, to_send, parse_mode="HTML", disable_web_page_preview=True)
